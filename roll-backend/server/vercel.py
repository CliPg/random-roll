...
class URL(server.SimpleHTTPRequestHandler):
    ...
    def translate_args(self):
        ...
        for word in words:
            ...
            if(len(word)==1):
                word.append('')
            # URL-decode key and value (support + as space)
            key = urllib.parse.unquote_plus(word[0])
            val = urllib.parse.unquote_plus(word[1])
            if(key in args.keys()):
...
#============ data translate ============#
    def translate_post(self):
        '区分 post 类型'
        # 安全读取 Content-Length（兼容大小写）
        length_header = None
        for key in ('Content-Length', 'content-length'):
            if key in self.headers:
                length_header = self.headers[key]
                break

        # 如果没有 Content-Length，或其值为 0，则把查询参数作为数据返回
        if not length_header:
            return self.translate_args()
        try:
            length = int(length_header)
        except (TypeError, ValueError):
            return self.translate_args()

        # 如果声明长度为 0，优先返回查询参数（常见于 DELETE 使用 query 的情况）
        if length == 0:
            return self.translate_args()
        ...
        if('Content-Type' in self.headers):
            ...
            if  (method == 'application/json'):
                try:
                    ...
                    # 数据不可解析为 JSON 时回退到查询参数（避免抛出导致连接中断）
                    return self.translate_args()
...
class register:
    ...
    def vercel(self, response, url, data, headers):
        ...
        try:
            ...
            # 在出现运行时错误时，应将 HTTP 请求处理器（response）传给 ErrorStatu，
            # 而不是装饰器对象 self（后者没有 send_response/send_text 等方法）。
            try:
                ErrorStatu(response, 500, 'Internal Server Error')
            except Exception:
                # 如果 response 未定义或不可用，退回到使用 register 本身（尽量不触发新的异常）
